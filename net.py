...
# https://github.com/MILVLG/bottom-up-attention.pytorch/tree/master
# Credit for the detectron2 usage
...
class DecoderRNN(nn.Module):
    def __init__(
        self, input_feature_dim, embed_size, hidden_size, vocab_size, num_layers=1
    ):
        super(DecoderRNN, self).__init__()

        ## Decoder parameters
        self.input_feature_dim = (
            input_feature_dim  # To project incoming features into embed size
        )
        ...
        # Projector from feature size to embedding size
        self.input_projector = nn.Linear(self.input_feature_dim, embed_size)
    ...
    def forward(self, features, captions):
        ...
        # Project
        features = self.input_projector(features)
        ...
        features = features.unsqueeze(1)  # So we do not depend on batch size
        if self.print_tensor_dimensions:
