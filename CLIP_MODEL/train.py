...
from models import CLIPEncoder, DecoderWithAttention
from datasets import CLIPCaptionDataset
from utils import *
...
clip_features_folder = 'features_clip'
use_subset = True
...
def main():
    ...
    if checkpoint is None:
        encoder = CLIPEncoder(clip_dim=clip_dim)
    ...
    train_dataset = CLIPCaptionDataset(
        ...
        data_name, 
        'TRAIN', 
        clip_features_folder=clip_features_folder,
...
def train(train_loader, encoder, decoder, criterion, encoder_optimizer, decoder_optimizer, epoch):
    ...
    start = time.time()
    for i, (clip_features, caps, caplens) in enumerate(train_loader):
        data_time.update(time.time() - start)
        clip_features = clip_features.to(device)
        caps = caps.to(device)
        caplens = caplens.to(device)
        imgs = encoder(clip_features)
...
def validate(val_loader, encoder, decoder, criterion):
    ...
    hypotheses = list()
    with torch.no_grad():
        for i, (clip_features, caps, caplens, allcaps) in enumerate(val_loader):
            clip_features = clip_features.to(device)
            caps = caps.to(device)
            caplens = caplens.to(device)
            imgs = encoder(clip_features)
