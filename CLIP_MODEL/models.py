...
class CLIPEncoder(nn.Module):
    def __init__(self, clip_dim=512, encoded_image_size=14):
        super(CLIPEncoder, self).__init__()
        self.enc_image_size = encoded_image_size
        self.clip_dim = clip_dim
        self.projection = nn.Linear(clip_dim, 2048)
        self.spatial_features = nn.Sequential(
            nn.Linear(clip_dim, encoded_image_size * encoded_image_size * 2048),
            nn.ReLU(),
            nn.Dropout(0.5)
        )
        self.init_weights()
    ...
    def init_weights(self):
        for layer in [self.projection, *self.spatial_features]:
    ...
    def forward(self, clip_features):
        ...
        spatial = self.spatial_features(clip_features)
        spatial = spatial.view(batch_size, self.enc_image_size, self.enc_image_size, 2048)
        return spatial
